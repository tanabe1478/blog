...
今後のuser-facing機能はPlaywright E2Eを必須にします。導入taskと全taskのDefinition of Doneは`cms-roadmap.md`を正本とします。

CMS-000完了後は、local Wranglerで実際のCMS HTML/JavaScriptを配信し、browserからの`/api/*`をPlaywrightでmockします。Production GitHubやGyazoへwriteせず、click、入力、navigation、reload、成功・競合・失敗表示をChromiumで検証します。
...
1. CMS-000: Playwright E2E基盤
2. CMS-001: localStorage自動下書きと復元
3. CMS-002以降をroadmap順に実装
