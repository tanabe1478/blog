...
1. 明示確認とSHA検証付きrename/delete
2. 保存後のGitHub Actions / GitHub Pages deploy status表示
3. 一覧のsearch/filter
4. PreviewのMarkdown対応拡張と公開rendererとの差の明示
5. fine-grained tokenの期限・rotation手順の定期確認

rename・deleteはwrite操作の範囲をさらに広げるため、既存のProduction-only制限、Origin確認、filename検証、SHA競合制御を再利用し、先にtestを追加します。
