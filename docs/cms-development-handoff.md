...
Detail取得はGitHub Contents APIを使い、Base64をUTF-8へdecodeします。保存時は取得時のBlob SHAを送ります。
...
CMSの「GitHubへ保存」が成功しても、GitHub Pages反映完了までは少し時間差があります。現在のCMSはdeploy完了statusを表示しません。
...
編集時だけ左Markdown・右Previewの2ペインになります。800px以下は縦1列です。
...
現在のtestはAccess認証、route、GitHub/Gyazo request、入力検証、競合、HTML内script構文等を確認します。外部APIはmockし、実repositoryやGyazoへ副作用を起こしません。
...
| `page.ts` | `npm run check`に加えagent-browserで実画面 |
...
優先度は次sessionでユーザーと確認します。

1. validated slug/frontmatterによる新規記事作成
2. 明示確認とSHA検証付きrename/delete
3. 保存後のGitHub Actions / GitHub Pages deploy status表示
4. 一覧のsearch/filter
5. PreviewのMarkdown対応拡張と公開rendererとの差の明示
6. fine-grained tokenの期限・rotation手順の定期確認

新規作成・rename・deleteはwrite操作の範囲を広げるため、既存のProduction-only制限、Origin確認、filename検証、SHA競合制御を再利用し、先にtestを追加します。
